...
static int parse_args(vector<const char*>& args, std::ostream *err_msg,
  ...
  if (args.begin() != args.end()) {
    if (strcmp(*args.begin(), "map") == 0) {
      ...
      cmd = List;
    } else if (strcmp(*args.begin(), "load-driver") == 0) {
      cmd = Load;
    } else if (strcmp(*args.begin(), "unload-driver") == 0) {
      cmd = Unload;
    } else {
...
static int load_driver()
{
    ...
    std::cout << "Driver location: " << driverLocation << std::endl;
    if (!ManageDriver(DRIVER_NAME,
        driverLocation,
        DRIVER_FUNC_INSTALL)) {

        std::cerr << "Unable to install driver." << std::endl;
